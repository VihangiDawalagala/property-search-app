// Image gallery component that displays property photos with navigation and lightbox view
...
export function ImageGallery({ images, alt }) {
  // Tracks which image is currently displayed as the main large image
  const [mainImage, setMainImage] = useState(0)
  // Controls whether the full-screen lightbox popup is open or closed
  const [isLightboxOpen, setIsLightboxOpen] = useState(false)
  // Tracks which image is shown in the lightbox popup
  const [lightboxIndex, setLightboxIndex] = useState(0)
  // Moves to the next image in the gallery, loops back to first if at end
  const nextImage = () => {
    setMainImage((prev) => (prev + 1) % images.length)
  }

  // Moves to the previous image in the gallery, loops to last if at beginning
  const prevImage = () => {
    setMainImage((prev) => (prev - 1 + images.length) % images.length)
  }

  // Moves to next image when viewing in full-screen lightbox mode
  const nextLightboxImage = () => {
    setLightboxIndex((prev) => (prev + 1) % images.length)
  }

  // Moves to previous image when viewing in full-screen lightbox mode
  const prevLightboxImage = () => {
    setLightboxIndex((prev) => (prev - 1 + images.length) % images.length)
  }

  // Opens the full-screen lightbox popup showing the clicked image
